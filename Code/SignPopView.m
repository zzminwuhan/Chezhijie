...
@interface SignPopView ()<UIGestureRecognizerDelegate>
...
+ (void)showInView:(UIView*)view shopModel:(ShopModel*)model success:(ActionBlock)success {
    ...
    SignPopView * popView = [[SignPopView alloc]initWithFrame:CGRectMake(0, 0, SCREEM_WIDTH, SCREEM_HEIGHT)];
...
- (void)addCustomView {
    ...
    [self setTextField:_textField3 placeholder:@"请输入收货地址"];
...
- (void)setTextField:(UITextField *)textField placeholder:(NSString*)placeholder {
    ...
    textField.leftView = [[UIView alloc]initWithFrame:CGRectMake(0, 0, 20, 30)];
