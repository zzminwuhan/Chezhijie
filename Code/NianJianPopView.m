...
@implementation NianJianPopView
...
+ (void)showInView:(UIView*)view shopModel:(ShopModel*)model success:(ActionBlock)success {
    ...
    NianJianPopView * popView = [[NianJianPopView alloc]initWithFrame:CGRectMake(0, 0, SCREEM_WIDTH, SCREEM_HEIGHT)];
...
- (void)detailWithModel:(ShopModel*)model {
    ...
    _label2.attributedText = [self showTitle:@"预约时间：" text:model.startdate];
    
    _label3.attributedText = [self showTitle:@"车牌号码：" text:model.carname];
    NSString *price = [NSString stringWithFormat:@"￥%@",model.price];
    _label4.attributedText = [self show2Title:@"付款金额：" text:price];
