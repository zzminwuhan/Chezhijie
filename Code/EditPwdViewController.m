...
- (void)initHeadView {
    ...
    _phone.title.text = @"旧密码";
    _phone.textField.placeholder = @"请输入旧密码";
    [_headView addSubview:_phone];
    
    _code = [[RBoxTextView alloc]initWithFrame:CGRectMake(0, y+44, SCREEM_WIDTH, 44)];
    _code.title.text = @"新密码";
    _code.textField.placeholder = @"输入新证码";
    [_headView addSubview:_code];
    
//    [_code setBottom];
    
    _password = [[RBoxTextView alloc]initWithFrame:CGRectMake(0, y+44*2, SCREEM_WIDTH, 44)];
    _password.title.text = @"确定密码";
    _password.textField.placeholder = @"请再次输入密码";
...
- (void)logBtnAction {
    ...
    if([_password.textField.text isEqualToString:_code.textField.text] == NO){
    ...
    NSString *url = HOSTAPIKEY(@"api/public/updatepwd");
    NSMutableDictionary *parameters = [NSMutableDictionary dictionary];
    
    NSString * mobile = _phone.textField.text;
    NSString * newpwd  = _password.textField.text;
//    NSString * code  = _password.textField.text;
    
    NSString * token = [UserManager getToken];
    [parameters setObject:token forKey:@"token"];
    
    [parameters setObject:mobile forKey:@"old"];
